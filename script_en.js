...
async function fetchApiKey() {
    ...
    try {
        ...
        alert('Could not load application settings. Weather functionality will be disabled. Please try again later.');
...
document.getElementById('weatherForm').addEventListener('submit', async function(event) {
    ...
    if (!cityInputVal || !/^[a-zA-Z\s,-]+$/.test(cityInputVal)) {
    ...
    try {
        ...
        if (!currentApiKey) {
            ...
            // alert("API settings are not available. Cannot search for weather.");
            return;
        }
...
const rateLimiter = {
    ...
    checkLimit() {
        ...
        if (now - this.lastCall < this.minInterval) {
            throw new Error('Please wait before making another search');
        }
        this.lastCall = now;
    }
};
...
function getWeather(city, currentApiKey) { // No longer async, key is passed in
    try {
        ...
        if(!currentApiKey) {
            ...
            alert('Critical error: Missing API key for weather search.');
            return;
        }
    
        const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&units=metric&lang=en&appid=${currentApiKey}`;
        const forecastUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${encodeURIComponent(city)}&units=metric&lang=en&appid=${currentApiKey}`;

        fetchWithTimeout(apiUrl)
            .then(response => {
                if (!response.ok) {
                    if (response.status === 401) throw new Error('Invalid or unauthorized API key from OpenWeatherMap.');
                    if (response.status === 404) throw new Error('City not found by OpenWeatherMap.');
            ...
            .then(data => {
                ...
                if (data.cod === 200) {
                    ...
                    throw new Error(sanitizeHTML('City not found or data error: ' + (data.message || 'Invalid response')));
                }
            })
            .then(forecastResponse => { // This .then is for the forecastUrl fetch
                ...
                if (!forecastResponse.ok) throw new Error('Error getting forecast from OpenWeatherMap.');
            ...
            .then(forecastData => {
                ...
                if (forecastData.cod === "200") {
                    ...
                    throw new Error(sanitizeHTML('Error getting forecast: ' + (forecastData.message || 'Invalid response')));
                }
            })
            .catch(error => { // Catch errors from any part of the promise chain
                console.error('Error in the weather retrieval process:', error);
...
function displayForecast(forecastData) {
    ...
    forecastItems.forEach(item => {
        ...
        const day = localForecastTime.toLocaleDateString('en-US', { weekday: 'short', timeZone: 'UTC' }); 
